Shader "Crystal/GrowCrystals"
{
	Properties
	{
        [Toggle]_FadeBody("Fade Body",float) = 0
        ...
        [Space(2)]
        [Toggle]_Crystal("grow Crystal",float) = 1
		_MaskTex ("MaskTexture", 2D) = "white" {}
        _Delta("Start position" , range(0.1,0)) = 0.001
	...
	SubShader
	{
		Tags { "RenderType"="Opaque" "LightMode" = "ForwardBase" "IgnoreProjector" = "True"}
		LOD 100
		...
		Pass{
				...
				float4 frag(v2f i) : SV_Target {
                    ...
                    float fade = ((mask)+_FadeBody>1);
                    clip(fade*-1);
                    
                    float3 col = tex2D(_MainTex,i.uv).rgb;
                    float3 normalmap = tex2D(_NormalMap,i.uv);
                    i.normal = i.normal + (normalmap - 0.5)*2;

                    float3 specularCol = _SpecularColor;


                    col = lerp(reflection(i,col,specularCol,_Specular),col,_Rate);
					return float4(col, 1);
				}
				ENDCG
		}
